...
function initwebdb(){
	read -p "The full path of uxdb installed[/home/uxdb/uxdbinstall]:" uxdb_path
	if [[ -z $uxdb_path ]]; then
...
function startwebdb(){
	webdbpath=$UXDBPATH
	read -p "The full path of uxdbwebdb[$UXDBPATH/uxdbwebdb]:" path_str
	if [[ -n $path_str ]]; then 
		if [[ ! -e $path_str ]]; then
	...
	export PATH=$PATH:$UXDBPATH/dbsql/bin
	echo "starting uxdbweb..................."
	nohup $UXDBPATH/dbsql/bin/ux_ctl start -D $webdbpath/uxdbwebdb/ >/dev/null 2>&1 &
	sleep 2
	echo "uxdbwebdb start success!"
}
function uxdbwebdb(){
	CUR_ROLE=`whoami`
	...
	echo -e "Init uxdbwebdb for uxdbweb server?[Y/N]:\c"
	read init_str
	...
	echo -e "Start uxdbwebdb for uxdbweb server?[Y/N]:\c"
	read start_str
